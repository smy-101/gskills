...
func TestFindSkillByName(t *testing.T) {
	tests := []struct {
		...
		skillName   string
		wantSkill   *types.SkillMetadata
		wantErr     bool
		errContains string
		teardown    func(registryPath string)
	}{
		{
			name: "skill found",
			setupFunc: func() string {
				...
				skills := []types.SkillMetadata{
					{
						...
						StorePath: filepath.Join(tmpDir, "skills", "test-skill"),
						Version:   "main",
						UpdatedAt: time.Now(),
					},
				}
				...
				skills := []types.SkillMetadata{
					{
						ID:        "other-skill@main",
						Name:      "other-skill",
						SourceURL: "https://github.com/test/other",
						StorePath: filepath.Join(tmpDir, "skills", "other-skill"),
						Version:   "main",
						UpdatedAt: time.Now(),
					},
				}
		...
		},
		{
			...
			wantSkill:   nil,
			wantErr:     true,
			errContains: "skill 'any-skill' not found",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			registryPath := tt.setupFunc()
			...
			gotSkill, err := findSkillByName(tt.skillName)
			...
			if gotSkill.ID != tt.wantSkill.ID {
				t.Errorf("ID = %v, want %v", gotSkill.ID, tt.wantSkill.ID)
			}
			if gotSkill.Name != tt.wantSkill.Name {
				t.Errorf("Name = %v, want %v", gotSkill.Name, tt.wantSkill.Name)
			}
			if gotSkill.SourceURL != tt.wantSkill.SourceURL {
				t.Errorf("SourceURL = %v, want %v", gotSkill.SourceURL, tt.wantSkill.SourceURL)
			}
			if gotSkill.Version != tt.wantSkill.Version {
				t.Errorf("Version = %v, want %v", gotSkill.Version, tt.wantSkill.Version)
			}
		})
	}
}
